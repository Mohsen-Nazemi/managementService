...
const os = require('os');
const numCPUs = os.cpus().length > 1 ? os.cpus().length : 1;
...
let frameworkConfig = {
  ...
  wantToUseFilterDataFunction: false,   // Note if you want to use filter data, <<<<PLEASE set it as 'FALSE'>>>> 
  filterRequestLogs: function (data) {
    /**
       ...
       * anything else...
       */
    data && data.hasOwnProperty('password') ? data.password = data.password.replace(/./g, '*') : true;
